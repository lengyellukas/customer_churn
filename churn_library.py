...
def encoder_helper(dataframe, category_list, response):
    ...
    logging.info(
        "turning each categorical column into a new column with propotion of churn")
...
def classification_report_image(y_train,
    ...
    logging.info(
        "The result has been stored in the file %s",
...
def train_models(X_train, X_test, y_train, y_test):
    ...
    logging.info(
        "Predicting using random forrest classifier using feature set")
    y_train_prediction_rf = cross_validation_rfc.best_estimator_.predict(
        X_train)
    ...
    plot_roc_curve(
        ...
        ax=ax,
        alpha=0.8)
    plot_roc_curve(cross_validation_rfc.best_estimator_,
